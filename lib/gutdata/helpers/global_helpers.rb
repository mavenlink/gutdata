...
module GutData
  module Helpers
    ...
    class << self
      ...
      # @return [Array<GutData::MdObject>] List of pairs of objects
      ...
      def deep_dup(an_object)
        case an_object
        when Array
          an_object.map { |it| GutData::Helpers.deep_dup(it) }
        when Hash
          an_object.each_with_object(an_object.dup) do |(key, value), hash|
            hash[GutData::Helpers.deep_dup(key)] = GutData::Helpers.deep_dup(value)
          end
        when Object
      ...
      def undot(params)
        ...
        params.map do |k, v|
          ...
          k.split('__').reverse.reduce(v) do |memo, obj|
            GutData::Helper.DeepMergeableHash[{ obj => memo }]
          end
        end
      end
