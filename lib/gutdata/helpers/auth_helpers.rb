...
module GutData
  module Helpers
    module AuthHelper
      class << self
        # Get path of .gutdata config
        def credentials_file
          "#{Helpers.home_directory}/.gutdata"
        end
        ...
        #
        # Tries to read it from ~/.gutdata file or from environment variable GD_SERVER
        # @param [String] credentials_file_path (credentials_file) Path to .gutdata file
        # @return [String] server token from environment variable, .gutdata or nil
        def read_environment(credentials_file_path = credentials_file)
          goodfile = read_credentials(credentials_file_path)
          [ENV['GD_ENVIRONMENT'], goodfile[:environment], GutData::Project::DEFAULT_ENVIRONMENT].find { |x| !x.nil? && !x.empty? }
        end
        ...
        # Tries to read it from ~/.gutdata file or from environment variable GD_SERVER
        # @param [String] credentials_file_path (credentials_file) Path to .gutdata file
        # @return [String] server token from environment variable, .gutdata or DEFAULT_URL
        def read_server(credentials_file_path = credentials_file)
          goodfile = read_credentials(credentials_file_path)
          [ENV['GD_SERVER'], goodfile[:server], GutData::Rest::Connection::DEFAULT_URL].find { |x| !x.nil? && !x.empty? }
        end

        # Try read token
        #
        # Tries to read it from ~/.gutdata file or from environment variable GD_PROJECT_TOKEN
        # @param [String] credentials_file_path (credentials_file) Path to .gutdata file
        # @return [String] auth token from environment variable, .gutdata or nil
