...
module GutData
  module Rest
    ...
    class Client
      ...
      # Constants
      #################################
      DEFAULT_CONNECTION_IMPLEMENTATION = GutData::Rest::Connection
      ...
      class << self
        ...
        def connect(username, password = 'aaaa', opts = {})
          ...
          username = GutData::Helpers.symbolize_keys(username) if username.is_a?(Hash)
        ...
        def retryable(options = {}, &block)
          GutData::Rest::Connection.retryable(options, &block)
        end
      ...
      # @option opts :connection_factory Object able to create new instances of GutData::Rest::Connection
      # @option opts [GutData::Rest::Connection] :connection Existing GutData::Rest::Connection
      def initialize(opts)
      ...
      def create_project(options = { title: 'Project' })
        GutData::Project.create({ client: self }.merge(options))
      end

      def create_project_from_blueprint(blueprint, options = {})
        GutData::Model::ProjectCreator.migrate(options.merge(spec: blueprint, client: self))
      end

      def domain(domain_name)
        GutData::Domain[domain_name, :client => self]
      end
      ...
      def projects(id = :all)
        GutData::Project[id, client: self]
      end

      def processes(id = :all)
        GutData::Process[id, client: self]
      end
      ...
      def user(id = nil)
        if id
          create(GutData::Profile, get(id))
        else
          create(GutData::Profile, @connection.user)
        end
      end
      ...
      def project_webdav_path(opts = { project: GutData.project })
        p = opts[:project]
        fail ArgumentError, 'No :project specified' if p.nil?

        project = GutData::Project[p, opts]
        fail ArgumentError, 'Wrong :project specified' if project.nil?

        url = project.links['uploads']
        fail 'Project WebDAV not supported in this Data Center' unless url

        GutData.logger.warn 'Beware! Project webdav is deprecated and should not be used.'
        url
      end
      ...
      def with_project(pid, &block)
        GutData.with_project(pid, client: self, &block)
      end

      def links
        GutData::Helpers.get_path(get('/gdc'), %w(about links))
      end
    end
  end
end
