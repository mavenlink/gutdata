...
module GutData
  module Rest
    module Connections
      # Implementation of GutData::Rest::Connection using https://rubygems.org/gems/rest-client
      class RestClientConnection < GutData::Rest::Connection
        def initialize(opts = {})
          super
        ...
        # @param uri [String] Target URI
        def get(uri, options = {})
          GutData.logger.debug "GET: #{@server.url}#{uri}"
          profile "GET #{uri}" do
        ...
        def put(uri, data, options = {})
          ...
          GutData.logger.debug "PUT: #{@server.url}#{uri}, #{scrub_params(data, [:password, :login, :authorizationToken, :verifyPassword])}"
          profile "PUT #{uri}" do
        ...
        def post(uri, data, options = {})
          GutData.logger.debug "POST: #{@server.url}#{uri}, #{scrub_params(data, [:password, :login, :authorizationToken, :verifyPassword])}"
          profile "POST #{uri}" do
            payload = data.is_a?(Hash) ? data.to_json : data
        ...
        def process_response(options = {}, &block)
          ...
          if content_type == 'application/json' || content_type == 'application/json;charset=UTF-8'
            ...
            GutData.logger.debug "Response: #{result.inspect}"
          elsif ['text/plain;charset=UTF-8', 'text/plain; charset=UTF-8', 'text/plain'].include?(content_type)
            result = response
            GutData.logger.debug "Response: plain text - #{result[0..99]}"
          ...
          GutData.logger.debug "Response: #{e.response}"
          raise $ERROR_INFO
        end
      end
    end
