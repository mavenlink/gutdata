...
module GutData
  class << self
    ...
    # Returns the active GutData connection earlier initialized via GutData.connect call
    #
    # @see GutData.connect
    def connection
      # TODO: Remove this after successful rest-factory transition
      Rest::Client.connection # || fail('Please authenticate with GutData.connect first')
    end

    alias_method :client, :connection

    # Connect to the GutData API
    #
    # @param options
    ...
    # Generates SSO URL
    #
    # This SSO implementation is custom implementation provided by GutData
    # that allows your application to sign in an existing GutData user.
    ...
    #
    # @param [String] login Email address used for logging into gutdata
    ...
    def sso_url(login, provider, opts = DEFAULT_SSO_OPTIONS)
      ...
      "#{GutData::Helpers::AuthHelper.read_server}/gdc/account/customerlogin?sessionId=#{CGI.escape(final)}&serverURL=#{CGI.escape(provider)}&targetURL=#{CGI.escape(opts[:url])}"
    end

    # Connect to GutData using SSO
    #
    # This SSO implementation is custom implementation provided by GutData
    # that allows your application to sign in an existing GutData user.
    # The authentication is done not by username and password but by generating a session
    ...
    # @param [String] login Email address used for logging into gutdata
    # @param [String] provider Name of SSO provider
    # @return [GutData::Rest::Client] Instance of REST client
