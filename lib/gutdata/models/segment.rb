  ...
  class Segment < Rest::Resource
    ...
    class << self
      ...
      def [](id, opts = {})
        ...
        if id == :all
          GutData::Segment.all(opts)
        else
          result = client.get(domain.segments_uri + "/segments/#{CGI.escape(id)}")
          client.create(GutData::Segment, result.merge('domain' => domain))
        end
      end
      ...
      def all(opts = {})
        ...
        GutData::Helpers.get_path(results, %w(segments items)).map { |i| client.create(GutData::Segment, i.merge('domain' => domain)) }
      end
      ...
      def create(data = {}, options = {})
        ...
        segment = client.create(GutData::Segment, GutData::Helpers.deep_stringify_keys(SEGMENT_TEMPLATE).merge('domain' => options[:domain]))
        segment.tap do |s|
    ...
    #
    # @return [GutData::Project] Project associated with the segment
    def master_project
    ...
    def create_client(data)
      client = GutData::Client.create(data, segment: self)
      client.save
    end
    ...
    def clients(tenant_id = :all)
      GutData::Client[tenant_id, domain: domain, segment: self]
    end

    # Creates or updates a segment instance on the API.
    #
    # @return [GutData::Segment] Segment instance
    def save
      if uri
        client.put(uri, json)
      else
    ...
    def delete(options = {})
      ...
      payload = GutData::Helpers.parse_http_exception(e)
      case GutData::Helpers.get_path(payload)
