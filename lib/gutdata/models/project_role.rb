  ...
  class ProjectRole < Rest::Resource
    ...
    def self.create_object(data)
      ...
      new_data = GutData::Helpers.deep_dup(EMPTY_OBJECT).tap do |d|
        ...
        d['projectRole']['meta'] = d['projectRole']['meta'].merge(GutData::Helpers.stringify_keys(meta_data))
        d['projectRole']['permissions'] = d['projectRole']['permissions'].merge(GutData::Helpers.stringify_keys(data[:permissions] || {}))
      end
      new(new_data)
    end
    ...
    def users
      ...
      tmp['associatedUsers']['users'].pmap do |user_url|
        url = user_url
        user = client.get url
        client.create(GutData::Profile, user)
      end
    end
