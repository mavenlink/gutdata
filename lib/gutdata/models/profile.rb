...
module GutData
  class Profile < Rest::Resource
    ...
    class << self
      ...
      # @option opts [GutData::Rest::Client] :client Client used for communication with server
      # @return GutData::Profile User Profile
      def [](id, opts = { client: GutData.connection })
        return id if id.instance_of?(GutData::Profile) || id.respond_to?(:profile?) && id.profile?
      ...
      # @return [GutData::Profile] New profile instance
      ...
      def create_object(attributes)
        json = GutData::Helpers.deep_dup(EMPTY_OBJECT)
        json['accountSetting']['links']['self'] = attributes[:uri] if attributes[:uri]
        res = client.create(GutData::Profile, json)
      ...
      # @return [GutData::Profile] User currently logged-in
      def current
        client.user
      end
    end
    ...
    #
    # @param right [GutData::Profile] Project to compare with
    ...
    # @return [GutData::Profile] Modified object
    # def apply(changes)
    #   GutData::Profile.apply(self, changes)
    # end
    ...
    #
    # @param user [GutData::Profile] Another profile to compare with
    # @return [Hash] Hash representing diff
    def diff(user)
      GutData::Profile.diff(self, user)
    end
    ...
    #
    # @return [Array<GutData::Project>] Array of project where account settings belongs to
    def projects
    ...
    def uri
      GutData::Helpers.get_path(@json, %w(accountSetting links self))
    end
    ...
    def to_hash
      tmp = GutData::Helpers.symbolize_keys(content.merge(uri: uri))
      [
        [:companyName, :company],
        [:phoneNumber, :phone],
