...
module GutData
  class MandatoryUserFilter < UserFilter
    class << self
      def [](id, options = { client: GutData.connection, project: GutData.project })
        if id == :all
          all(options)
        else
          super
      ...
      def all(options = { client: GutData.connection, project: GutData.project })
        ...
        mufs = filters.map do |filter_data|
          ...
          c.create(GutData::MandatoryUserFilter, payload, project: project)
        end
        mufs.enum_for
      end

      def count(options = { client: GutData.connection, project: GutData.project })
        c = client(options)
        project = options[:project]
    ...
    #
    # @return [GutData::MandatoryUserFilter]
    def save
      data = {
        'userFilter' => {
          'content' => {
