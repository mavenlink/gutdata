...
module GutData
  class UserFilter < GutData::MdObject
    def initialize(data)
      @dirty = false
      @json = GutData::Helpers.symbolize_keys(data)
    end
    ...
    #
    # @return [GutData::Project | GutData::Profile] Related object
    def related
      uri = related_uri
      level == :project ? client.projects(uri) : client.create(GutData::Profile, client.get(uri))
    end
    ...
    def variable
      uri = @json[:prompt]
      GutData::Variable[uri, client: client, project: project]
    end
