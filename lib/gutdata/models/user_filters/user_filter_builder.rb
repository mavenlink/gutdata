...
module GutData
  module UserFilterBuilder
    ...
    def self.execute_mufs(user_filters, options = {})
      ...
      fail GutData::FilterMaqlizationError, errors if !ignore_missing_values && !errors.empty?
      ...
      GutData.logger.warn("Data permissions computed: #{to_create.count} to create and #{to_delete.count} to delete")
      ...
      create_results = to_create.each_slice(100).flat_map do |batch|
        batch.pmapcat do |related_uri, group|
          ...
          res['userFiltersUpdateResult'].flat_map { |k, v| v.map { |r| { status: k.to_sym, user: r, type: :create } } }.map { |result| result[:status] == :failed ? result.merge(GutData::Helpers.symbolize_keys(result[:user])) : result }
        end
      end
                           ...
                           batch.flat_map do |related_uri, group|
                             ...
                             if related_uri
                               ...
                               results.concat(res['userFiltersUpdateResult']
                                 ...
                                 .map { |result| result[:status] == :failed ? result.merge(GutData::Helpers.symbolize_keys(result[:user])) : result })
                             end
                             group.peach(&:delete)
                             results
                           end
