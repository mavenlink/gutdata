...
##
# Module containing classes that counter-part GutData server-side meta-data
...
module GutData
  module Model
    ...
    class << self
      def title(item)
        item[:title] || GutData::Helpers.titleize(item[:id])
      end
      ...
      def upload_data(path, project_blueprint, dataset, options = { :client => GutData.connection, :project => GutData.project })
        data = [
          {
            data: path,
            dataset: dataset,
        ...
        GutData::Model.upload_multiple_data(data, project_blueprint, options)
      end
      ...
      def upload_multiple_data(data, project_blueprint, options = { :client => GutData.connection, :project => GutData.project })
        client, project = GutData.get_client_and_project(options)

        project ||= GutData.project

        manifest = {

          'dataSetSLIManifestList' => data.map do |d|
            mode = d[:options] && d[:options][:mode] ? d[:options][:mode] : options[:mode] || 'FULL'
            GutData::Model::ToManifest.dataset_to_manifest(project_blueprint, d[:dataset], mode)
          end
        }
        ...
        if res['wTaskStatus']['status'] == 'ERROR'
          ...
          messages.each do |msg|
            GutData.logger.error(JSON.pretty_generate(msg))
          end
      ...
      def merge_dataset_columns(a_schema_blueprint, b_schema_blueprint)
        ...
        d = GutData::Helpers.deep_dup(a_schema_blueprint)
        ...
        unless columns_that_failed_to_merge.empty?
          columns_that_failed_to_merge.each do |error|
            GutData.logger.error "Columns #{error[0]} failed to merge. There are #{error[1]} conflicting columns. When merging columns with the same name they have to be identical."
            GutData.logger.error error[2]
          end
