...
module GutData
  module Model
    class DatasetBlueprint < SchemaBlueprint
      ...
      # or the GutData::Model::DatasetBlueprint
      #
      ...
      #
      # @param dataset [Hash | GutData::Model::ProjectBlueprint] Dataset blueprint
      ...
      # Returns attributes and anchor defined on a dataset
      #
      # @return [Array<GutData::Model::DatasetBlueprint>] returns the attributes
      def attributes_and_anchors
        attributes + [anchor]
      end

      # Changes the dataset through a builder. You provide a block and an istance of
      # GutData::Model::SchemaBuilder is passed in as the only parameter
      #
      # @return [GutData::Model::SchemaBlueprint] returns changed dataset blueprint
      def change(&block)
      ...
      def columns
        DatasetBlueprint.columns(to_hash).map do |c|
          case c[:type].to_sym
          when :anchor
            GutData::Model::AnchorBlueprintField.new(c, self)
          when :attribute
            GutData::Model::AttributeBlueprintField.new(c, self)
          when :fact
            GutData::Model::FactBlueprintField.new(c, self)
          when :label
            GutData::Model::LabelBlueprintField.new(c, self)
          when :reference
            GutData::Model::ReferenceBlueprintField.new(c, self)
          when :date
            GutData::Model::ReferenceBlueprintField.new(c, self)
          else
            GutData::Model::BlueprintField.new(c, self)
          end
        end
      end
      ...
      #
      # @return [GutData::Model::DatasetBlueprint] matching fields
      def dup
        DatasetBlueprint.new(GutData::Helpers.deep_dup(data), project_blueprint)
      end
      ...
      #
      # @param col [GutData::Model::BlueprintField | Hash] Field
      # @return [GutData::Model::BlueprintField] matching fields
      def find_column(col)
      ...
      # @return [GutData::Model::BlueprintField] returns matching field
      ...
      # @param type [String | Symbol | Array[Symmbol] | Array[String]] Type or types you would like to get
      # as third parameter it return all object otherwise it returns the first one
      # @return [Array<GutData::Model::BlueprintField>] matching fields
      ...
      # @return [GutData::Model::DatasetBlueprint] returns itself changed
      def merge!(a_blueprint)
        new_blueprint = GutData::Model.merge_dataset_columns(self, a_blueprint)
        @data = new_blueprint
        self
      end
      ...
      #
      # @return [Array<GutData::Metric>] matching fields
      def suggest_metrics
        identifiers = facts.map { |f| identifier_for(f) }
        identifiers.zip(facts).map do |id, fact|
          Metric.xcreate(
            :title => GutData::Helpers.titleize(fact[:name]),
      ...
      def to_blueprint
        GutData::Model::ProjectBlueprint.new(datasets: [to_hash])
      end
      ...
      def validate_gd_data_type_errors
        (labels + facts)
          .select { |x| x.gd_data_type && !GutData::Model.check_gd_data_type(x.gd_data_type) }
