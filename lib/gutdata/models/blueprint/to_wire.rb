    ...
    module ToWire
        ...
        labels = ls.map do |l|
          {
            label: {
              identifier: l[:id],
              title: GutData::Model.title(l),
              type: l[:gd_type] || Model::DEFAULT_TYPE,
              dataType: GutData::Model.normalize_gd_data_type(l[:gd_data_type]) || Model::DEFAULT_ATTRIBUTE_DATATYPE
            }
          }
        end
        {}.tap do |a|
          ...
          a[:attribute][:folder] = attribute[:folder] || dataset[:folder] || GutData::Model.title(dataset)
          a[:attribute][:labels] = labels unless labels.empty?
          a[:attribute][:description] = GutData::Model.description(attribute) if GutData::Model.description(attribute)
          if attribute[:grain]
      ...
      def self.dataset_to_wire(project, dataset)
        {
          dataset: {
            ...
            title: GutData::Model.title(dataset),
      ...
      def self.date_dimension_to_wire(_project, dataset)
        payload = {}.tap do |dd|
          ...
          dd[:title] = GutData::Model.title(dataset)
        end
      ...
      def self.fact_to_wire(dataset, fact)
        payload = {
          fact: {
            identifier: fact[:id],
            title: GutData::Model.title(fact),
            folder: fact[:folder] || dataset[:folder] || GutData::Model.title(dataset),
            dataType: GutData::Model.normalize_gd_data_type(fact[:gd_data_type]) || DEFAULT_FACT_DATATYPE
          }
        }
        payload.tap do |p|
          p[:fact][:description] = GutData::Model.description(fact) if GutData::Model.description(fact)
        end
      end
