...
module GutData
  module Model
    module ToManifest
      ...
      #
      # @param project [GutData::Model::ProjectBlueprint] Project blueprint
      # @param dataset [GutData::Model::SchemaBlueprint] Dataset blueprint
      ...
      def self.date_ref_to_manifest(project, _dataset, reference, mode)
        ...
        format = reference[:format] || GutData::Model::DEFAULT_DATE_FORMAT
        [{
          'populates' => [ref],
          'mode' => mode,
      ...
      def self.to_manifest(project, mode = 'FULL')
        ProjectBlueprint.datasets(project.to_hash).map do |dataset|
          columns = GutData::Model::DatasetBlueprint.columns(dataset)
          {
            'dataSetSLIManifest' => {
