...
module GutData
  class Project < Rest::Resource
    ...
    EMPTY_OBJECT = {
      'project' => {
        ...
        'content' => {
          'guidedNavigation' => 1,
          'driver' => 'Pg',
          'environment' => GutData::Helpers::AuthHelper.read_environment
        }
      }
    }
    ...
    class << self
      ...
      # current user
      def all(opts = { client: GutData.connection })
        c = client(opts)
        c.user.projects
      end
      ...
      def [](id, opts = { client: GutData.connection })
        return id if id.instance_of?(GutData::Project) || id.respond_to?(:project?) && id.project?

        if id == :all
          Project.all({ client: GutData.connection }.merge(opts))
        else
      ...
      def find(_opts = {}, client = GutData::Rest::Client.client)
        user = client.user
        user.projects['projects'].map do |project|
          client.create(GutData::Project, project)
        end
      end

      def create_from_blueprint(blueprint, options = {})
        GutData::Model::ProjectCreator.migrate(options.merge(spec: blueprint, client: GutData.connection))
      end
    ...
    def add_dashboard(dashboard)
      GutData::Dashboard.create(dashboard, :client => client, :project => self)
    end

    alias_method :create_dashboard, :add_dashboard

    def add_user_group(data)
      g = GutData::UserGroup.create(data.merge(project: self))
    ...
    # @return [GutData::Report] Instance of new report
    def add_metric(metric, options = {})
      default = { client: client, project: self }
      if metric.is_a?(String)
        GutData::Metric.xcreate(metric, options.merge(default))
      else
        GutData::Metric.xcreate(options[:expression], metric.merge(options.merge(default)))
      end
    end
    ...
    def add_report(options = {})
      rep = GutData::Report.create(options.merge(client: client, project: self))
      rep.save
    end
    ...
    # This report definition can be used for creating of GutData::Report
    #
    # @param [json] Raw report definition json
    # @return [GutData::ReportDefinition] Instance of new report definition
    def add_report_definition(json)
      rd = GutData::ReportDefinition.new(json)
      rd.client = client
      rd.project = self
      rd.save
    end
    ...
    #
    # @param [String | Number | Object] Anything that you can pass to GutData::Attribute[id]
    # @return [GutData::Attribute | Array<GutData::Attribute>] fact instance or list
    def attributes(id = :all)
      GutData::Attribute[id, project: self, client: client]
    end

    def attribute_by_identifier(identifier)
      GutData::Attribute.find_first_by_identifier(identifier, project: self, client: client)
    end

    def attributes_by_identifier(identifier)
      GutData::Attribute.find_by_identifier(identifier, project: self, client: client)
    end

    def attribute_by_title(title)
      GutData::Attribute.find_first_by_title(title, project: self, client: client)
    end

    def attributes_by_title(title)
      GutData::Attribute.find_by_title(title, project: self, client: client)
    end

    # Gets project blueprint from the server
    #
    # @return [GutData::ProjectRole] Project role if found
    def blueprint(options = {})
      ...
      bp = GutData::Model::FromWire.from_wire(model)
      bp.title = title
      bp
    end
    ...
    # @return [GutData::Project] Newly created project
    def clone(options = {})
      ...
      begin
        ...
        new_project = GutData::Project.create(options.merge(:title => a_title, :client => client, :driver => content[:driver]))
    ...
    # data server.
    #
    # @param id [Symbol | String | GutData::MdObject] Export options
    # @return [Array<GutData::Dataset> | GutData::Dataset] Dataset or list of datasets in the project
    def datasets(id = :all)
      GutData::Dataset[id, project: self, client: client]
    end

    def dimensions(id = :all)
      GutData::Dimension[id, client: client, project: self]
    end
    ...
    def user_groups(id = :all, options = {})
      GutData::UserGroup[id, options.merge(project: self)]
    end
    ...
    def compute_report(spec = {})
      GutData::ReportDefinition.execute(spec.merge(client: client, project: self))
    end

    def compute_metric(expression)
      GutData::Metric.xexecute(expression, client: client, project: self)
    end

    alias_method :compute_measure, :compute_metric

    def create_schedule(process, date, executable, options = {})
      s = GutData::Schedule.create(process, date, executable, options.merge(client: client, project: self))
      s.save
    end

    def create_variable(data)
      GutData::Variable.create(data, client: client, project: self)
    end

    # Helper for getting dashboards of a project
    #
    # @param id [String | Number | Object] Anything that you can pass to GutData::Dashboard[id]
    # @return [GutData::Dashboard | Array<GutData::Dashboard>] dashboard instance or list
    ...
    def deploy_process(path, options = {})
      GutData::Process.deploy(path, options.merge(client: client, project: self))
    end
    ...
    def execute_maql(maql, options = {})
      ldm_links = client.get(md[GutData::Model::LDM_CTG])
      ldm_uri = Links.new(ldm_links)[GutData::Model::LDM_MANAGE_CTG]
    ...
    #
    # @param [String | Number | Object] Anything that you can pass to GutData::Fact[id]
    # @return [GutData::Fact | Array<GutData::Fact>] fact instance or list
    def facts(id = :all)
      GutData::Fact[id, project: self, client: client]
    end

    def fact_by_title(title)
      GutData::Fact.find_first_by_title(title, project: self, client: client)
    end

    def facts_by_title(title)
      GutData::Fact.find_by_title(title, project: self, client: client)
    end

    def find_attribute_element_value(uri)
      GutData::Attribute.find_element_value(uri, client: client, project: self)
    end
    ...
    def get_role(role_name, role_list = roles)
      return role_name if role_name.is_a? GutData::ProjectRole
    ...
    def upload_file(file, options = {})
      GutData.upload_to_project_webdav(file, options.merge(project: self))
    end

    def download_file(file, where)
      GutData.download_from_project_webdav(file, where, project: self)
    end
    ...
    def fuzzy_get_user(name, user_list = users)
      return name if name.instance_of?(GutData::Membership)
      return member(name) if name.instance_of?(GutData::Profile)
    ...
    #
    # @param user [GutData::Profile | GutData::Membership | String] User in question. Can be passed by login (String), profile or membershi objects
    # @param role_name [String || GutData::ProjectRole] Project role cna be given by either string or GutData::ProjectRole object
    ...
    # GutData::Label[id] + it supports :all as welll
    # @return [GutData::Fact | Array<GutData::Fact>] fact instance or list
    def labels(id = :all, opts = {})
      if id == :all
        attributes.pmapcat(&:labels).uniq
      else
        GutData::Label[id, opts.merge(project: self, client: client)]
      end
    end
    ...
    #
    # @return [Array<GutData::Metric>] matric instance or list
    def metrics(id = :all, opts = { :full => true })
      GutData::Metric[id, opts.merge(project: self, client: client)]
    end

    alias_method :measures, :metrics

    def metric_by_title(title)
      GutData::Metric.find_first_by_title(title, project: self, client: client)
    end

    alias_method :measure_by_title, :metric_by_title

    def metrics_by_title(title)
      GutData::Metric.find_by_title(title, project: self, client: client)
    end
    ...
    #
    # @return [Array<GutData::MdObject>] object instance or list
    def objects(id, opts = {})
      GutData::MdObject[id, opts.merge(project: self, client: client)]
    end

    # Transfer objects from one project to another
    #
    # @param [Array<GutData::MdObject | String>, String, GutData::MdObject] objs Any representation of the object or a list of those
    # @param [Hash] options The options to migration.
    # @option options [Number] :time_limit Time in seconds before the blocking call will fail. See GutData::Rest::Client.poll_on_response for additional details
    ...
    def objects_export(objs, options = {})
      ...
      if polling_result['wTaskStatus'] && polling_result['wTaskStatus']['status'] == 'ERROR'
        messages = GutData::Helpers.interpolate_error_messages(polling_result['wTaskStatus']['messages']).join(' ')
    ...
    # Transfer objects from one project to another
    #
    # @param [Array<GutData::MdObject | String>, String, GutData::MdObject] objects Any representation of the object or a list of those
    # @param [Hash] options The options to migration.
    # @option options [GutData::Project | String | Array<String> | Array<GutData::Project>] :project Project(s) to migrate to
      ...
      if projects.is_a?(Array)
        projects.each_slice(batch_size).flat_map do |batch|
          batch.pmap do |proj|
            ...
            rescue GutData::ObjectsImportError => e
              {
                project: target_project,
                result: false,
                reason: e.message
    ...
    #
    # @param [String | Number | Object] Anything that you can pass to GutData::Report[id]
    # @return [GutData::Report | Array<GutData::Report>] report instance or list
    def processes(id = :all)
      GutData::Process[id, project: self, client: client]
    end
    ...
    def info
      ...
      puts title
      puts GutData::Helpers.underline(title)
      puts
      ...
      results.each do |x|
        ...
        dataset.title.tap do |t|
          puts t
          puts GutData::Helpers.underline(t)
    ...
    #
    # @param mapping [Array<Array>] Mapping specifying what should be exchanged for what. As mapping should be used output of GutData::Helpers.prepare_mapping.
    ...
    # @return [GutData::Report | Array<GutData::Report>] report instance or list
    def reports(id = :all)
      GutData::Report[id, project: self, client: client]
    end

    # Helper for getting report definitions of a project
    #
    # @param [String | Number | Object] Anything that you can pass to GutData::ReportDefinition[id]
    # @return [GutData::ReportDefinition | Array<GutData::ReportDefinition>] report definition instance or list
    def report_definitions(id = :all, options = {})
      GutData::ReportDefinition[id, options.merge(project: self, client: client)]
    end

    # Gets the list or project roles
    #
    # @return [Array<GutData::ProjectRole>] List of roles
    def roles
      ...
      tmp['projectRoles']['roles'].pmap do |role_url|
        ...
        client.create(GutData::ProjectRole, json, project: self)
      end
    end

    # Saves project
    def save
      data_to_send = GutData::Helpers.deep_dup(raw_data)
    ...
    def schedule_mail(options = GutData::ScheduledMail::DEFAULT_OPTS)
      GutData::ScheduledMail.create(options.merge(client: client, project: self))
    end

    def scheduled_mails(options = { :full => false })
      GutData::ScheduledMail[:all, options.merge(project: self, client: client)]
    end

    # @param [String | Number | Object] Anything that you can pass to GutData::Schedule[id]
    # @return [GutData::Schedule | Array<GutData::Schedule>] schedule instance or list
    def schedules(id = :all)
      GutData::Schedule[id, project: self, client: client]
    end

    # Gets SLIs data
    #
    # @return [GutData::Metadata] SLI Metadata
    def slis
    ...
    def upload(data, blueprint, dataset_name, options = {})
      GutData::Model.upload_data(data, blueprint, dataset_name, options.merge(client: client, project: self))
    end

    def upload_multiple(data, blueprint, options = {})
      GutData::Model.upload_multiple_data(data, blueprint, options.merge(client: client, project: self))
    end
    ...
    #
    # @return [Array<GutData::MandatoryUserFilter>] List of mandatory user
    def user_filters
      url = "/gdc/md/#{pid}/userfilters"

      tmp = client.get(url)
      tmp['userFilters']['items'].pmap do |filter|
        client.create(GutData::MandatoryUserFilter, filter, project: self)
      end
    end
    ...
    #
    #
    # @return [Array<GutData::User>] List of users
    def users(opts = {})
      client = client(opts)
      Enumerator.new do |y|
        ...
        loop do
          ...
          tmp['users'].each do |user_data|
            user = client.create(GutData::Membership, user_data, project: self)
    ...
    def import_users(new_users, options = {})
      ...
      GutData.logger.warn("Importing users to project (#{pid})")
      ...
      diff = GutData::Helpers.diff(whitelisted_users, diffable_new, key: :login, fields: [:login, :role, :status])

      # Create new users
      u = diff[:added].map { |x| { user: x, role: x[:role] } }

      results = []
      GutData.logger.warn("Creating #{diff[:added].count} users in project (#{pid})")
      results.concat(create_users(u, roles: role_list, project_users: whitelisted_users))

      # # Update existing users
      GutData.logger.warn("Updating #{diff[:changed].count} users in project (#{pid})")
      ...
      GutData.logger.warn("Removing #{to_remove.count} users from project (#{pid})")
    ...
    def set_user_roles(login, desired_roles, options = {})
      ...
      failure = GutData::Helpers.get_path(res, %w(projectUsersUpdateResult failed))
    ...
    def set_users_roles(list, options = {})
      ...
      typed_results = results.flat_map { |x| x['projectUsersUpdateResult'].flat_map { |k, v| v.map { |v_2| v_2.is_a?(String) ? { type: k.to_sym, user: v_2 } : GutData::Helpers.symbolize_keys(v_2).merge(type: k.to_sym) } } }
    ...
    def add_data_permissions(filters, options = {})
      GutData::UserFilterBuilder.execute_mufs(filters, { client: client, project: self }.merge(options))
    end

    def add_variable_permissions(filters, var, options = {})
      GutData::UserFilterBuilder.execute_variables(filters, var, { client: client, project: self }.merge(options))
    end
    ...
    def variables(id = :all, options = { client: client, project: self })
      GutData::Variable[id, options]
    end

    def update_from_blueprint(blueprint, options = {})
      GutData::Model::ProjectCreator.migrate(options.merge(spec: blueprint, token: options[:auth_token], client: client, project: self))
    end
