...
module GutData
  class Membership < Rest::Resource
    ...
    class << self
      ...
      #
      # @param obj [GutData::User] Object to be modified
      # @param changes [Hash] Hash with modifications
      # @return [GutData::User] Modified object
      ...
      #   obj
      # end
      def create(data, options = { client: GutData.connection })
        c = client(options)
        json = {
          'user' => {
            'content' => {
      ...
      def diff_list(list_1, list_2)
        GutData::Helpers.diff(list_1, list_2, key: :login)
      end
    end
    ...
    #
    # @param right [GutData::User] Project to compare with
    ...
    # @return [GutData::User] Modified object
    # def apply(changes)
    #   GutData::User.apply(self, changes)
    # end
    ...
    def contributor
      ...
      data = client.get url
      client.create(GutData::Membership, data)
    end
    ...
    #
    # @param user [GutData::User] Another profile to compare with
    # @return [Hash] Hash representing diff
    def diff(user)
      GutData::User.diff(self, user)
    end
    ...
    def profile
      ...
      client.create(GutData::Profile, raw)
    end
    ...
    #   client.create(GutData::Project, raw)
    # end
    ...
    #
    # @return [Array<GutData::Project>] Array of projets
    def projects
    ...
    # Gets first role
    #
    # @return [GutData::ProjectRole] Array of project roles
    def role
      roles && roles.first
    end
    ...
    #
    # @return [Array<GutData::ProjectRole>] Array of project roles
    def roles
      roles_link = GutData::Helpers.get_path(@json, %w(user links roles))
      return unless roles_link
      tmp = client.get roles_link
      tmp['associatedRoles']['roles'].pmap do |role_uri|
        role = client.get role_uri
        client.create(GutData::ProjectRole, role)
      end
    end
    ...
    # Enables membership
    #
    # @return [GutData::Membership] returns self
    def enable
      self.status = 'ENABLED'
      self
    end
    ...
    def to_hash
      tmp = GutData::Helpers.symbolize_keys(content.merge(meta).merge('uri' => uri))
      [
        [:userRoles, :role],
