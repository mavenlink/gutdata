...
module GutData
  class ScheduledMail < GutData::MdObject
    root_key :scheduledMail

    include GutData::Mixin::Lockable
    ...
    class << self
      ...
      def convert_attachment(item, opts)
        if item.is_a?(GutData::Dashboard)
          {
            dashboardAttachment: GutData::DashboardAttachment::DEFAULT_OPTS.merge(opts.merge(:uri => item.uri))
          }
        elsif item.is_a?(GutData::Report)
          {
            reportAttachment: GutData::ReportAttachment::DEFAULT_OPTS.merge(opts.merge(:uri => item.uri))
          }
        elsif item.is_a?(GutData::DashboardAttachment)
          item.json
        elsif item.is_a?(GutData::ReportAttachment)
          item.json
        elsif item.is_a?(Hash)
          item
        elsif item == 'dashboardAttachment'
          {
            dashboardAttachment: GutData::DashboardAttachment::DEFAULT_OPTS.merge(opts)
          }
        elsif item == 'reportAttachment'
          {
            reportAttachment: GutData::ReportAttachment::DEFAULT_OPTS.merge(opts)
          }
        end
      end
      ...
      def create(options = { :client => GutData.connection, :project => GutData.project })
        ...
        opts = GutData::ScheduledMail::DEFAULT_OPTS.merge(GutData::Helpers.symbolize_keys(options))
        ...
        client.create(ScheduledMail, GutData::Helpers.deep_stringify_keys(scheduled_mail), :project => project)
      end
    end
    ...
    #
    # @return [Array<GutData::DashboardAttachment | GutData::ReportAttachment>] Array of attachments
    def attachments
      content['attachments'].map do |attachment|
        key = attachment.keys.first

        if key == 'dashboardAttachment'
          GutData::DashboardAttachment.new(self, attachment)
        elsif key == 'reportAttachment'
          GutData::ReportAttachment.new(self, attachment)
        else
