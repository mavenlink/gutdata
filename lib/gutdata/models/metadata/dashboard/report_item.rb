...
module GutData
  class ReportItem < DashboardItem
    ...
    class << self
      ...
      def create(tab, item)
        res = GutData::ReportItem.new(tab, GutData::Helpers.deep_dup(GutData::Helpers.deep_stringify_keys(EMPTY_OBJECT)))
        item.each do |k, v|
    ...
    def obj
      GutData::MdObject[data['obj'], :client => tab.dashboard.client, :project => tab.dashboard.project]
    end
