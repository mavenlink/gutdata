...
module GutData
  # Report Definition
  # TODO: Add more doc ...
  class ReportDefinition < GutData::MdObject
    class << self
      ...
      #
      # @param filters [GutData::Variable|Array<Array>]
      # @param options [Hash] the options hash
      ...
      def create_filters_part(filters, options = {})
        ...
        vars = filters.select { |f| f.is_a?(GutData::Variable) }.map { |v| { expression: "[#{v.uri}]" } }
        category = filters.select { |f| f.is_a?(Array) }.map { |v| GutData::SmallGoodZilla.create_category_filter(v, project) }
        vars + category
      end
      ...
      def find(stuff, opts = { :client => GutData.connection, :project => GutData.project })
        _client, project = GutData.get_client_and_project(opts)
        ...
        stuff.map do |item|
          obj = if item.is_a?(String)
                  ...
                  case item[:type].to_s
                  when 'metric'
                    GutData::Metric.find_first_by_title(item[:title], opts)
                  when 'attribute'
                    GutData::Attribute.find_first_by_title(item[:title], opts)
                  end
      ...
      def execute(options = {})
        ...
        begin
          ...
          GutData::ReportDefinition.create(options).execute
        ensure
    ...
    #
    # @return [GutData::ReportDefinition] Returns self
    def reset_color_mapping!
      global_chart_options = GutData::Helpers.get_path(content, %w(chart styles global))
    ...
    def execute(opts = {})
      ...
      GutData::Report.data_result(result, opts.merge(client: client))
    end
    ...
    # @return [GutData::ReportDefinition]
    def replace(mapping)
