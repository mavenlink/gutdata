...
module GutData
  module Model
    class ProjectCreator
      class << self
        def migrate(opts = {})
          opts = { client: GutData.connection }.merge(opts)
          client = opts[:client]
          fail ArgumentError, 'No :client specified' if client.nil?

          spec = opts[:spec] || fail('You need to provide spec for migration')
          bp = ProjectBlueprint.new(spec)
          fail GutData::ValidationError, "Blueprint is invalid #{bp.validate.inspect}" unless bp.valid?
          spec = bp.to_hash
        ...
        def migrate_datasets(spec, opts = {})
          ...
          while response.code != 200
            sleep 1
            GutData::Rest::Client.retryable(:tries => 3) do
              sleep 1
              response = client.get(link, :process => false)
            end
          end
          ...
          if !chunks.nil? && !dry_run
            chunks['updateScript']['maqlDdlChunks'].each do |chunk|
              GutData.logger.info "Migrating DDL Chunk for #{project.pid}: #{chunk}" if opts[:verbose]
        ...
        def load(project, spec)
          if spec.key?(:uploads) # rubocop:disable Style/GuardClause
            spec[:uploads].each do |load|
              schema = GutData::Model::Schema.new(spec[:datasets].find { |d| d[:name] == load[:dataset] })
        ...
        def execute_tests(_project, spec)
          spec.each do |assert|
            result = GutData::ReportDefinition.execute(assert[:report])
        ...
        def pick_correct_chunks(chunks, opts = {})
          ...
          results = GutData::Helpers.join(rules, stuff, [:cascade_drops, :preserve_data], [:cascade_drops, :preserve_data], inner: true).sort_by { |l| l[:priority] }
