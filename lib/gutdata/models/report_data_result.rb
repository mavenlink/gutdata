...
module GutData
  class ReportDataResult < Rest::Resource
    class << self
      ...
      # @return [GutData::ReportDataResult] Returns new report data result
      def from_xtab(data)
        top = top_headers(data)
        left = left_headers(data)
        jank = GutData::Helpers.zeroes(rows(top), cols(left), nil)
        d = data(data)
        stuff = d.empty? ? GutData::Helpers.zeroes(rows(left), cols(top), nil) : d
    ...
    #
    # @return [GutData::ReportDataResult] Returns new report data result with flipped columns and rows
    def transpose
    ...
    # @return [GutData::ReportDataResult] Returns new report data result sliced data
    def slice(rows, cols)
    ...
    #
    # @param [GutData::ReportDataResult] other The other report result
