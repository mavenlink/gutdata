  ...
  class Process < Rest::Resource
    ...
    class << self
      ...
      def with_deploy(dir, options = {}, &block)
        _client, project = GutData.get_client_and_project(options)

        GutData.with_project(project) do
          params = options[:params].nil? ? [] : [options[:params]]
          if block
            begin
              res = GutData::Process.deploy(dir, options.merge(:files_to_exclude => params))
              block.call(res)
            rescue => e
              puts e.inspect
            ensure
            ...
            GutData::Process.deploy(dir, options.merge(:files_to_exclude => params))
          end
        end
      end

      def upload_package(path, files_to_exclude, opts = { :client => GutData.connection })
      ...
      def deploy(path, options = { :client => GutData.client, :project => GutData.project })
    ...
    def execute(executable, options = {})
      ...
      client.create(GutData::ExecutionDetail, result, client: client, project: project)
    end
