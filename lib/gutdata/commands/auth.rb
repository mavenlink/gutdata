    ...
    class Auth
      class << self
        ...
        def ask_for_credentials(credentials_file_path = Helpers::AuthHelper.credentials_file)
          puts 'Enter your GutData credentials.'

          old_credentials = Helpers::AuthHelper.read_credentials(credentials_file_path)

          # Ask for user email
          user = GutData::CLI.terminal.ask('Email') do |q|
            set_default_value(q, old_credentials[:username])
          end

          # Ask for password
          password = GutData::CLI.terminal.ask('Password') do |q|
            # set_default_value(q, old_credentials[:password])
            q.echo = '*'
          end

          # Ask for token
          auth_token = GutData::CLI.terminal.ask('Authorization Token') do |q|
            set_default_value(q, old_credentials[:auth_token])
          end

          # Read environment
          environment = GutData::CLI.terminal.ask('Environment') do |q|
            set_default_value(q, old_credentials[:environment], GutData::Project::DEFAULT_ENVIRONMENT)
          end

          # Read server
          server = GutData::CLI.terminal.ask('Server') do |q|
        ...
        def store(credentials_file_path = Helpers::AuthHelper.credentials_file)
          puts 'This will store credntials to GutData in an enencrypted form to your harddrive to file ~/.gutdata.'
          overwrite = GutData::CLI.terminal.ask('Do you want to continue? (y/n)')
          return if overwrite != 'y'

          credentials = ask_for_credentials

          ovewrite = if File.exist?(credentials_file_path)
                       GutData::CLI.terminal.ask('Overwrite existing stored credentials (y/n)')
                     else
                       'y'
                     end
