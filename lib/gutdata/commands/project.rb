    ...
    class Project
      class << self
        ...
        def create(options = { client: GutData.connection })
          ...
          GutData::Project.create(:title => title, :summary => summary, :template => template, :auth_token => token, :client => client, :driver => driver)
        end

        # Show existing project
        def show(id, options = { client: GutData.connection })
          client = options[:client]
          client.projects(id)
        end

        def invite(project_id, email, role, msg = GutData::Project::DEFAULT_INVITE_MESSAGE, options = {})
          client = options[:client]
        ...
        #
        # @param project_id [String | GutData::Project] Project id or project instance to delete
        ...
        def clone(project_id, options = { client: GutData.connection })
          client = options[:client]
        ...
        #
        # @param project_id [String | GutData::Project] Project id or project instance to delete
        def delete(project_id, options = { client: GutData.connection })
          client = options[:client]
        ...
        def get_spec_and_project_id(base_path)
          goodfile_path = GutData::Helpers.find_goodfile(Pathname(base_path))
        ...
        # Update project
        def update(opts = { client: GutData.connection })
          client, project = GutData.get_client_and_project(opts)
          GutData::Model::ProjectCreator.migrate(:spec => opts[:spec], :client => client, :project => project)
        end

        # Build project
        def build(opts = { client: GutData.connection })
          client = opts[:client]
          fail ArgumentError, 'No :client specified' if client.nil?

          GutData::Model::ProjectCreator.migrate(opts.merge(:client => client))
        end

        # Performs project validation
        #
        # @param project_id [String | GutData::Project] Project id or project instance to validate
        # @return [Object] Report of found problems
        def validate(project_id, options = { client: GutData.connection })
          client = options[:client]
          client.with_project(project_id, &:validate)
        end

        def jack_in(options)
          goodfile_path = GutData::Helpers.find_goodfile(Pathname('.'))
          ...
          spin_session = proc do |goodfile, _blueprint|
            ...
            begin
              require 'gutdata'
              client = GutData.connect(options)
            ...
            rescue GutData::ProjectNotFound
          ...
          if goodfile_path
            ...
            blueprint = GutData::Model::ProjectBlueprint.new(eval(File.read(model_key)).to_hash) if File.exist?(model_key) && !File.directory?(model_key)
        ...
        #
        # @param project_id [String | GutData::Project] Project id or project instance to list the users in
        # @return [Array <GutData::Role>] List of project roles
        def roles(project_id, options = { client: GutData.connection })
          client = options[:client]
        ...
        #
        # @param project_id [String | GutData::Project] Project id or project instance to list the users in
        # @return [Array <GutData::Membership>] List of project users
        def users(project_id, options = { client: GutData.connection })
          client = options[:client]
