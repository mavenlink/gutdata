...
module GutData
  module Mixin
    module MdObjectQuery
      ...
      def all(_options = { :client => GutData.connection, :project => GutData.project })
      ...
      # @option options [Boolean] :full if passed true the subclass can decide to pull in full objects. This is desirable from the usability POV but unfortunately has negative impact on performance so it is not the default
      # @return [Array<GutData::MdObject> | Array<Hash>] Return the appropriate metadata objects or their representation
      def query(query_obj_type, klass, options = { :client => GutData.connection, :project => GutData.project })
        ...
        project = GutData::Project[p, options]
      ...
      def dependency(uri, key = nil, opts = { :client => GutData.connection })
        c = opts[:client]
        ...
        if opts[:full]
          result = result.map do |res|
            GutData::MdObject[res['link'], :client => c, :project => opts[:project]]
          end
        end
      ...
      def dependency?(type, uri, target_uri, opts = { :client => GutData.connection, :project => GutData.project })
      ...
      def usedby(uri, key = nil, opts = { :client => GutData.connection, :project => GutData.project })
        p = opts[:project]
        fail ArgumentError, 'No :project specified' if p.nil?
      ...
      def using(uri, key = nil, opts = { :client => GutData.connection, :project => GutData.project })
        p = opts[:project]
        fail ArgumentError, 'No :project specified' if p.nil?
      ...
      def usedby?(uri, target_uri, opts = { :client => GutData.connection, :project => GutData.project })
      ...
      def using?(uri, target_uri, opts = { :client => GutData.connection, :project => GutData.project })
