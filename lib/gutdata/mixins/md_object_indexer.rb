  ...
  module Mixin
    module MdObjectIndexer
      ...
      def [](id, options = { :client => GutData.connection, :project => GutData.project })
        ...
        # new(GutData.get uri) unless uri.nil?
        if uri # rubocop:disable Style/GuardClause
          raw = client.get(uri)
          md_class = self
          case raw.keys.first
          when 'attribute'
            md_class = GutData::Attribute
          when 'metric'
            md_class = GutData::Metric
          when 'projectDashboard'
            md_class = GutData::Dashboard
          when 'report'
            md_class = GutData::Report
          when 'attributeDisplayForm'
            md_class = GutData::Label
