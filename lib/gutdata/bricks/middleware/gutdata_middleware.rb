...
module GutData
  module Bricks
    class GutDataMiddleware < Bricks::Middleware
      def call(params)
        ...
        client = if params['GDC_USERNAME'].nil? || params['GDC_PASSWORD'].nil?
                   ...
                   GutData.connect(sst_token: params[token_name], server: server)
                 else
                   puts "Connecting as #{params['GDC_USERNAME']} to server #{server}"
                   GutData.connect(params['GDC_USERNAME'], params['GDC_PASSWORD'], server: server)
                 end
        project = client.projects(project_id)
        GutData.project = project
        GutData.logger = logger
