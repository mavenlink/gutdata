  ...
  module SmallGoodZilla
    class << self
      ...
      def create_category_filter(spec, project)
        ...
        label = item.is_a?(GutData::Attribute) ? item.primary_label : item
      ...
      def pretty_print(expression, opts = { client: GutData.connection, project: GutData.project })
        temp = expression.dup
        pairs = get_uris(expression).pmap do |uri|
          if uri =~ /elements/
            ...
            [uri, GutData::MdObject[uri, opts].title]
          end
        end
        pairs.each do |el|
          uri = el[0]
      ...
      def interpolate_ids(options, *ids)
        ...
        if ids.empty?
          ...
          res = GutData::MdObject.identifier_to_uri(options, *ids)
      ...
      def interpolate_metric(metric, dictionary, options = { :client => GutData.connection, :project => GutData.project })
        interpolated = interpolate({
                                     :facts => GutData::SmallGoodZilla.get_facts(metric),
                                     :attributes => GutData::SmallGoodZilla.get_attributes(metric),
                                     :metrics => GutData::SmallGoodZilla.get_metrics(metric)
                                   }, dictionary)

        ids = GutData::SmallGoodZilla.get_ids(metric)
